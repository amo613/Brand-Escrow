...
export const api = {
  ...
  socials: () => req('/api/users/socials'),
  socialStart: (platform: string) => req(`/api/social/${platform}/start`),
  airdrop: (role?: string) => req('/api/users/airdrop', { method: 'POST', body: JSON.stringify({ role }) }),
  deals: () => req('/api/deals'),
  deal: (id: string) => req(`/api/deals/${id}`),
  registerDeal: (d: any) => req('/api/deals', { method: 'POST', body: JSON.stringify(d) }),
  apply: (id: string, handle: string) => req(`/api/deals/${id}/apply`, { method: 'POST', body: JSON.stringify({ handle }) }),
